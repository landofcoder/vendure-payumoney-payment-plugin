...
import { getGateway } from '../helpers/payumoney-common';
...
export class PayumoneyResolver {
    ...
    @Query()
    async generatePayumoneyClientToken(@Ctx() ctx: RequestContext, @Args() orderId: ID) {
    ...
    private async getPaymentMethodArgs(): Promise<PaymentMethodArgsHash> {
        ...
        if (!method) {
            throw new InternalServerError(`[${loggerCtx}] Could not find Payumoney PaymentMethod`);
        }
